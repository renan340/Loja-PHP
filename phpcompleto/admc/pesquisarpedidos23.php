...
<table class="table table-strip">
	...
	$preco   = number_format ($in ['precototal'], 2,',','.');
	$sub     = number_format ($in ['precototal'] * $qtd, 2,',','.');
	$total   += $in['precototal'] * $qtd;
	$codcliente = $_SESSION['cliente'];
	$date = date('Y-m-d');
	$datacompra = $date;
	$sql22 = "insert into pedido values (null,'".$codcliente."', '".$codproduto."','".$qtd."','".$precototal."', '".$total."', '".$datacompra."')";
